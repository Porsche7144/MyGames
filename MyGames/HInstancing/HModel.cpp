...
bool HModel::Init()
{
	...
	m_InstanceCount = 4;
	m_Instance = new HInstance[m_InstanceCount];
	if (!m_Instance)
	{
		return false;
	}
	...
	D3D11_BUFFER_DESC instanceBufferDesc =
	{
		...
		sizeof(HInstance) * m_InstanceCount,
		D3D11_USAGE_DYNAMIC,
	...
	if (FAILED(HBASIS_CORE_LIB::g_pd3dDevice->CreateBuffer(&instanceBufferDesc, &instanceData, m_pInstanceBuffer.GetAddressOf())))
	{
		return false;
	}
...
bool HModel::Render(ID3D11DeviceContext* pContext)
{
	...
	for (int iNode = 0; iNode < m_pFbxObj->m_hNodeList.size(); iNode++)
	{
		...
		Matrix MatrixWorld, matScale, matRotate, matTrans;
		matRotate = m_matView.Invert();
		matRotate._41 = 1.0f;
		matRotate._42 = 1.0f;
		matRotate._43 = 1.0f;
		matRotate._44 = 1.0f;

		for (int i = 0; i < pObject->m_AnimList.size(); i++)
		{
			pObject->m_AnimList[i].p += m_pFbxObj->m_fTick * pObject->m_AnimList[i].s;
			pObject->m_AnimList[i].iTick += m_pFbxObj->m_fTick;

			matTrans.Translation(pObject->m_AnimList[i].p);
			m_InstanceList[i].matWorld = matRotate * matTrans;

		}

		if (SUCCEEDED(pContext->Map((ID3D11Resource*)m_pInstanceBuffer.Get(), 0,
			D3D11_MAP_WRITE_DISCARD, 0, &MappedFaceDest)))
		{

			// m_Instance->mat = (Matrix*)MappedFaceDest.pData;
			// memcpy(instance, m_pFbxObj->m_MatrixList)

			for (int iObj = 0; iObj < m_pFbxObj->m_hNodeList.size(); iObj++)
			{

				// matTrans._41 = m_Instance->mat[iObj]._41;
				// matTrans._42 = m_Instance->mat[iObj]._42;
				// matTrans._43 = m_Instance->mat[iObj]._43;
				...
				MatrixWorld = matTrans * matRotate * matAnim;
				//m_Instance->mat[iObj] = MatrixWorld.Transpose();
				//m_InstanceList.push_back(m_Instance->mat[iObj]);
			}
		...
		for (int iSub = 0; iSub < pObject->m_SubMesh.size(); iSub++)
		{
			...
			//////////////////////////////
			// 멀티스트림 = 버퍼 다중사용.
			ID3D11Buffer* vb[2] = { pMesh->m_pVertexBuffer.Get(), m_pInstanceBuffer.Get() };
			UINT iStride[2] = { sizeof(PNCT_VERTEX), sizeof(HInstance) };
			UINT iOffset[2] = { 0,0 };
			...
			else
			{
				...
				pContext->DrawIndexedInstanced(pMesh->m_IndexArray.size(), m_InstanceCount, 0, 0, 0);
			}
		}
	}
