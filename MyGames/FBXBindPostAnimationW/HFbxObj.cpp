		...
		for (int iTri = 0; iTri < iTriangleCount; iTri++)
		{
			...
			for (int iIndex = 0; iIndex < 3; iIndex++)
			{
				...
				if (VertexTangentSet.size())
				{
					...
					{
						FbxVector4 tangent = ReadTangent(pMesh, VertexTangentSet.size(), TangentElement, 
											iCornerIndices[iIndex], iBasePolyIndex + iVertIndex[iIndex]);
...
FbxVector4 HFbxObj::ReadTangent(const FbxMesh* mesh,
