	...
	if (m_pFbxObj->Load(m_ObjName))
	{
		...
		for (auto data : m_pFbxObj->m_hNodeList)
		{
			...
			for (int iSub = 0; iSub < pObject->m_SubMesh.size(); iSub++)
			{
				...
				for (int i = 0; i < pSub->m_VertexList.size(); i++)
				{

					if (TopXYZ.x < pSub->m_VertexList[i].p.x)
					{
						TopXYZ.x = pSub->m_VertexList[i].p.x;
					}
					if (TopXYZ.y < pSub->m_VertexList[i].p.y)
					{
						TopXYZ.y = pSub->m_VertexList[i].p.y;
					}
					if (TopXYZ.z < pSub->m_VertexList[i].p.z)
					{
						TopXYZ.z = pSub->m_VertexList[i].p.z;
					}

					if (BottomXYZ.x > pSub->m_VertexList[i].p.x)
					{
						BottomXYZ.x = pSub->m_VertexList[i].p.x;
					}
					if (BottomXYZ.y > pSub->m_VertexList[i].p.y)
					{
						BottomXYZ.y = pSub->m_VertexList[i].p.y;
					}
					if (BottomXYZ.z > pSub->m_VertexList[i].p.z)
					{
						BottomXYZ.z = pSub->m_VertexList[i].p.z;
					}

					//beforeXYZ = pSub->m_VertexList[i].p;
				}
