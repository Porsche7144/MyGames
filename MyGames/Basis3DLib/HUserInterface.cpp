...
#include "HScene.h"
#include "HWrite.h"
int HEdit::g_iControlID = 1000;

LRESULT	 HUserInterface::MsgProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	for (auto iter : m_pChildObjects)
	{
		HUserInterface* pUI = (HUserInterface*)iter;
		int iRet = pUI->MsgProc(hWnd, msg, wParam, lParam);
		if (iRet >= 0) return iRet;
	}
	return -1;
}
HObject* HUserInterface::Clone()
{
	return new HUserInterface(*this);
}
bool  HUserInterface::PostFrame()
{
	return true;
}
HPoint  HActor::Gen(float x, float y)
{
	HPoint vRet(x, y);
	//// 0 ~ 800 - > 0 ~1 -> -1 ~ +1
	//vRet.x = x / 800;
...
void    HActor::GenCenter()
{
	m_vCenter.x = 0.0f;
	m_vCenter.y = 0.0f;
	for (int iV = 0; iV < 6; iV++)
	{
		HPoint vertex = m_VertexList[iV].p;
...
bool    HActor::SetData(float left, float top, float width, float height)
{
	m_rtInit.left = left;
...
void   HActor::SetVertexData()
{
	HPoint pos = Gen(m_rtInit.left, m_rtInit.top);
	m_VertexList[0].p = HPoint(pos.x, pos.y);
	m_VertexList[0].t = HPoint(0.0f, 0.0f);
	pos = Gen(m_rtInit.left + m_rtInit.right, m_rtInit.top);
	m_VertexList[1].p = HPoint(pos.x, pos.y);
	m_VertexList[1].t = HPoint(1.0f, 0.0f);
	pos = Gen(m_rtInit.left, m_rtInit.top + m_rtInit.bottom);
	m_VertexList[2].p = HPoint(pos.x, pos.y);
	m_VertexList[2].t = HPoint(0.0f, 1.0f);
	...
	m_VertexList[5].p = HPoint(pos.x, pos.y);
	m_VertexList[5].t = HPoint(1.0f, 1.0f);
	GenCenter();
}
bool    HDialogBox::SetData(RECT rt, float fScaleX0, float fScaleX1,
...
void  HDialogBox::Set(RECT rtSrc, RECT rtDesk)
{
	HObject::Set(rtSrc, rtDesk);
...
void  HDialogBox::Set(HPoint p, RECT rtSrc)
{
	HObject::Set(p, rtSrc);
	SetData(m_rtDesk, 0.1f, 0.1f,     // 화면 좌우 스케일 00, 02
		0.1f, 0.1f,		// 화면 상하 스케일 00, 20
...
bool  HDialogBox::Render(ID3D11DeviceContext*	pd3dContext)
{
	//for (int iRect = 0; iRect < 9; iRect++)
	//{
	//	RECT rt;
...
bool   HButton::Render(ID3D11DeviceContext*	pd3dContext)
{
...
LRESULT	 HEdit::MsgProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	switch (msg)
	{
		case WM_LBUTTONDOWN:
		{
			...
			if (HCollision::RectInPt(g_rtClient, g_Input.GetPos()))
			{
				if (!HCollision::RectInPt(m_rtWndCtrl, g_Input.GetPos()))
				{
					::SetFocus(g_hWnd);
					m_bEditFocus = false;
					g_Input.SetEnable();
...
HObject* HEdit::Clone()
{
	return new HEdit(*this);
}
void   HEdit::CreateEdit()
{
...
bool   HEdit::Render(ID3D11DeviceContext*	pd3dContext)
{
	RECT rtDraw = m_rtDesk;
	if (m_pParentObject != nullptr)
	{
