...
namespace DX
{
	ID3D11RasterizerState* HDxState::g_pRSWireFrame = 0;
	ID3D11RasterizerState* HDxState::g_pRSSolid = 0;
	ID3D11RasterizerState* HDxState::g_pRSBackCullSolid = 0;
	ID3D11RasterizerState* HDxState::g_pRSNoneCullSolid = 0;
	ID3D11RasterizerState* HDxState::g_pRSFrontCullSolid = 0;
	ID3D11RasterizerState* HDxState::g_pRSSlopeScaledDepthBias;
	ID3D11RasterizerState* HDxState::g_pRS[6] = { 0, };

	ID3D11SamplerState*	 HDxState::g_pTexSS = 0;
	ID3D11SamplerState*  HDxState::g_pSSWrapLinear = NULL;
	ID3D11SamplerState*  HDxState::g_pSSWrapPoint = NULL;
	ID3D11SamplerState*  HDxState::g_pSSMirrorLinear = NULL;
	ID3D11SamplerState*  HDxState::g_pSSMirrorPoint = NULL;
	ID3D11SamplerState*  HDxState::g_pSSClampLinear = NULL;
	ID3D11SamplerState*  HDxState::g_pSSClampPoint = NULL;
	ID3D11SamplerState*  HDxState::g_pSSShadowMap = NULL;
	ID3D11SamplerState*  HDxState::g_pSS[8] = { 0, };

	ID3D11DepthStencilState*	HDxState::g_pDSSDepthEnable = 0;
	ID3D11DepthStencilState*	HDxState::g_pDSSDepthDisable = 0;
	ID3D11DepthStencilState*	HDxState::g_pDSSDepthEnableNoWrite = 0;
	ID3D11DepthStencilState*	HDxState::g_pDSSDepthDisableNoWrite = 0;
	ID3D11DepthStencilState*	HDxState::g_pDSSDepthStencilAdd = 0;
	ID3D11DepthStencilState*	HDxState::g_pDSSDepthAlways = 0;
	ID3D11DepthStencilState*	HDxState::g_pDSS[6] = { 0, };

	ID3D11BlendState*	  HDxState::g_pAlphaBlend = 0;
	ID3D11BlendState*	  HDxState::g_pNoAlphaBlend = 0;
	ID3D11BlendState*	  HDxState::g_pBSColorOne = 0;
	ID3D11BlendState*	  HDxState::g_pBSOneOne = 0;
	ID3D11BlendState*	  HDxState::g_pBSOneZero = 0;
	ID3D11BlendState*	  HDxState::g_pBSAlphaOne = 0;
	ID3D11BlendState*	  HDxState::g_pBSMaxOneOne = 0;
	ID3D11BlendState*	  HDxState::g_pBS[7] = { 0, };

	Microsoft::WRL::ComPtr<ID3D11BlendState>		HDxState::g_pBSEdit = 0;
	Microsoft::WRL::ComPtr<ID3D11RasterizerState>	HDxState::g_pRSEdit = 0;
	Microsoft::WRL::ComPtr<ID3D11SamplerState>      HDxState::g_pSSEdit = 0;
	Microsoft::WRL::ComPtr<ID3D11DepthStencilState> HDxState::g_pDSSEdit = 0;
	D3D11_BLEND_DESC			HDxState::g_BlendDesc = { 0, };
	D3D11_RASTERIZER_DESC		HDxState::g_RasterizerDesc;
	D3D11_SAMPLER_DESC			HDxState::g_SamplerDesc;
	D3D11_DEPTH_STENCIL_DESC	HDxState::g_DepthStencilDesc = { 0, };

	FLOAT HDxState::g_fBlendFactor[4] = { 0, };
	UINT  HDxState::g_iMask = 0xff;
	UINT HDxState::g_iSlot = 0;
	UINT HDxState::g_iArray = 1;
	UINT HDxState::g_iRef = 1;

	HRESULT HDxState::SetBlendState(ID3D11Device* pd3dDevice,
	...
	HRESULT HDxState::SetRasterizerState(ID3D11Device* pd3dDevice,
		ID3D11DeviceContext* pContext,
	...
	HRESULT HDxState::SetSamplerState(ID3D11Device* pd3dDevice,
		ID3D11DeviceContext* pContext,
	...
	HRESULT HDxState::SetDipthStencilState(ID3D11Device* pd3dDevice,
		ID3D11DeviceContext* pContext,
	...
	HRESULT HDxState::SetState(ID3D11Device*	pd3dDevice)
	{
		...
		pd3dDevice->CreateBlendState(&blendDesc, &HDxState::g_pBSColorOne);

		blendDesc.RenderTarget[0].BlendOp = D3D11_BLEND_OP_ADD;// D3D11_BLEND_OP_REV_SUBTRACT;
		blendDesc.RenderTarget[0].SrcBlend = D3D11_BLEND_ONE;// DEST_COLOR;// D3D11_BLEND_DEST_COLOR;
		blendDesc.RenderTarget[0].DestBlend = D3D11_BLEND_ONE;// DEST_COLOR;
		pd3dDevice->CreateBlendState(&blendDesc, &HDxState::g_pBSOneOne);

		blendDesc.RenderTarget[0].SrcBlend = D3D11_BLEND_ONE;// DEST_COLOR;// D3D11_BLEND_DEST_COLOR;
		blendDesc.RenderTarget[0].DestBlend = D3D11_BLEND_ZERO;// DEST_COLOR;
		pd3dDevice->CreateBlendState(&blendDesc, &HDxState::g_pBSOneZero);

		blendDesc.RenderTarget[0].SrcBlend = D3D11_BLEND_SRC_ALPHA;
		blendDesc.RenderTarget[0].DestBlend = D3D11_BLEND_ONE;
		pd3dDevice->CreateBlendState(&blendDesc, &HDxState::g_pBSAlphaOne);

		blendDesc.AlphaToCoverageEnable = TRUE;
		blendDesc.RenderTarget[0].BlendOp = D3D11_BLEND_OP_MAX;
		blendDesc.RenderTarget[0].SrcBlend = D3D11_BLEND_ONE;
		blendDesc.RenderTarget[0].DestBlend = D3D11_BLEND_ONE;
		pd3dDevice->CreateBlendState(&blendDesc, &HDxState::g_pBSMaxOneOne);
		...
		if (FAILED(hr = pd3dDevice->CreateRasterizerState(&rsDesc, &HDxState::g_pRSBackCullSolid))) return hr;
		g_RasterizerDesc = rsDesc;

		rsDesc.FillMode = D3D11_FILL_WIREFRAME;
		rsDesc.CullMode = D3D11_CULL_NONE;
		if (FAILED(hr = pd3dDevice->CreateRasterizerState(&rsDesc, &HDxState::g_pRSWireFrame)))
			return hr;
		rsDesc.FillMode = D3D11_FILL_SOLID;
		rsDesc.CullMode = D3D11_CULL_BACK;
		if (FAILED(hr = pd3dDevice->CreateRasterizerState(&rsDesc, &HDxState::g_pRSSolid)))
			return hr;

		rsDesc.FillMode = D3D11_FILL_SOLID;
		rsDesc.CullMode = D3D11_CULL_NONE;
		if (FAILED(hr = pd3dDevice->CreateRasterizerState(&rsDesc, &HDxState::g_pRSNoneCullSolid)))
			return hr;


		rsDesc.FillMode = D3D11_FILL_SOLID;
		rsDesc.CullMode = D3D11_CULL_FRONT;
		if (FAILED(hr = pd3dDevice->CreateRasterizerState(&rsDesc, &HDxState::g_pRSFrontCullSolid)))
			return hr;
		...
		if (FAILED(hr = pd3dDevice->CreateRasterizerState(&rsDesc, &HDxState::g_pRSSlopeScaledDepthBias)))
			return hr;
		...
		g_DepthStencilDesc = dsDescDepth;
		// 깊이 버퍼 비교 비 활성화.
