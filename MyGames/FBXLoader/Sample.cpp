...
bool Sample::Init()
{
	...
	{
		"../../Image/FBX_Image/man.fbx",
	};

	for (int iObj = 0; iObj < ARRAYSIZE(fbxobject); iObj++)
	{
		std::shared_ptr<HFbxObj> obj = make_shared<HFbxObj>();
		if (obj->Load(fbxobject[iObj]))
		{
			for (auto data : obj->m_hMeshList)
			{
				HModelObject* pObject = data;
...
bool Sample::Render()
{
	for (int iObj = 0; iObj < m_ObjList.size(); iObj++)
	{
		m_ObjList[iObj]->m_fTick += g_fSecondPerFrame *
			m_ObjList[iObj]->m_AnimScene.iFrameSpeed *
			m_ObjList[iObj]->m_AnimScene.iTickPerFrame;// *0.1f;

		if (m_ObjList[iObj]->m_fTick >=
			(m_ObjList[iObj]->m_AnimScene.iEndFrame *
				m_ObjList[iObj]->m_AnimScene.iTickPerFrame))
		{
			m_ObjList[iObj]->m_fTick = 0;
		}
		for (auto data : m_ObjList[iObj]->m_hMeshList)
		{
			Matrix matWorld = Matrix::Identity;
			HModelObject* pObject = data;
			for (int iTick = 0; iTick < pObject->m_AnimList.size(); iTick++)
			{
				if (pObject->m_AnimList[iTick].iTick >= m_ObjList[iObj]->m_fTick)
				{
					matWorld = pObject->m_AnimList[iTick].mat;
					break;
				}
			}
